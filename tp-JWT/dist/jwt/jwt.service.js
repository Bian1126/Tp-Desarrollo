...
let JwtService = class JwtService {
    ...
    refreshToken(refreshToken) {
        try {
            ...
            return {
                accessToken: this.generateToken({ id: payload.id, email: payload.email }, 'auth'),
                refreshToken: timeToExpire < 20
                    ? this.generateToken({ id: payload.id, email: payload.email }, 'refresh')
                    : refreshToken
            };
        }
        catch (error) {
    ...
    getPayload(token, type = 'auth') {
        ...
        if (typeof decoded === 'string') {
            throw new common_1.UnauthorizedException('Invalid token payload format');
        }
        if (!('email' in decoded)) {
